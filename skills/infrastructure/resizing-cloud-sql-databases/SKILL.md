...
**ALL 8 steps required. NO exceptions. NO emergencies. NO shortcuts.**
...
- ❌ "This is a safe operation" - FOLLOW ALL 8 STEPS ANYWAY
...
| Step | Command | Wait? |
|------|---------|-------|
| Stop services | `gcloud app versions stop` | No |
| Create backup | `gcloud sql backups create` | YES - wait for DONE |
| Patch instance | `gcloud sql instances patch` | YES - wait for DONE |
| Verify config | `gcloud sql instances describe` | No |
| Start services | `gcloud app versions start` | YES - wait for SERVING |
| Verify services | `gcloud app versions list` | No |
...
# 3. Wait for backup (output showed "done")
